...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `Animateur_different_participant` BEFORE INSERT ON `participer_session` FOR EACH ROW if (NEW.Numero_AGENT = (select Numéro_Animateur from session  where Numéro_Session = New.Numero_Session )) then
SIGNAL SQLSTATE '45000'
SET MESSAGE_TEXT = 'Votre agent est déjà animateur dans cette session';
END IF */;;
DELIMITER ;
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `Update_Animateur` BEFORE UPDATE ON `session` FOR EACH ROW if (new.numéro_animateur = (select numero_agent from participer_session)) then
signal SQLSTATE '45000'
set message_text = 'Votre agent participe déjà à cette session';
END IF */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
-- Dump completed on 2022-03-22 10:06:02
